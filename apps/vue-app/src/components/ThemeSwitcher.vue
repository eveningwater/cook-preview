<template>
  <div ref="themeSwitcherRef" class="theme-switcher">
    <div class="theme-buttons">
      <button
        class="theme-button"
        :class="{ active: themeService.currentThemeRef.value === 'light' }"
      ...
      <button
        class="theme-button"
        :class="{ active: themeService.currentThemeRef.value === 'dark' }"
      ...
      <button
        class="theme-button"
        :class="{ active: themeService.currentThemeRef.value === 'custom' }"
    ...
    <transition name="slide">
      <div v-if="showCustomThemes && themeService.currentThemeRef.value === 'custom'" class="custom-themes">
...
import type { ThemeMode } from '@cook/core';
...
// 监听主题切换
watch(() => themeService.currentThemeRef.value, (newTheme) => {
  if (newTheme === 'custom') {
    showCustomThemes.value = true;
  } else {
    showCustomThemes.value = false;
...
onMounted(() => {
  ...
  // 如果当前是自定义主题，显示面板
  if (themeService.currentThemeRef.value === 'custom') {
    showCustomThemes.value = true;
  }
});
